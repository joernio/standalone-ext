...
object Accessors {
  ...
  final class Access_Property_MYPROPERTY(val node: nodes.StoredNode) extends AnyVal {
    def myproperty: String =
      flatgraph.Accessors.getNodePropertySingle(node.graph, node.nodeKind, 39, node.seq(), "": String)
  }
  ...
  final class Access_MynodetypeBase(val node: nodes.MynodetypeBase) extends AnyVal {
    def myproperty: String = node match {
      case stored: nodes.StoredNode     => new Access_Property_MYPROPERTY(stored).myproperty
...
trait ConcreteStoredConversions extends ConcreteBaseConversions {
  ...
  ): Access_Property_MYPROPERTY = new Access_Property_MYPROPERTY(node)
...
trait ConcreteBaseConversions extends AbstractBaseConversions0 {
  ...
  implicit def access_MynodetypeBase(node: nodes.MynodetypeBase): Access_MynodetypeBase = new Access_MynodetypeBase(
    node
  )
